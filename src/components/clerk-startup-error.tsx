    ...
    <SafeAreaView style={authStyles.safe}>
      <ScrollView contentContainerStyle={authStyles.scroll}>
        ...
        <Text style={[authStyles.subtitle, { marginTop: 16 }]}>
          Use the same Clerk app as the web admin: production key (pk_live_…) on EAS preview, Convex
          CLERK_JWT_ISSUER_DOMAIN=https://clerk.sdvedutech.in, and Clerk Dashboard → Integrations → Convex → Activate.
          Then rebuild: npm run eas:build:android:preview
        </Text>
        <Text style={[authStyles.subtitle, { marginTop: 12 }]}>
          Web app: set NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY to the same pk_live_… as this mobile app (not a different
          pk_test_ instance).
        </Text>
      </ScrollView>
    </SafeAreaView>
