...
async function querySurveysByMunicipality(
  ...
  const q = status
    ...
    : ctx.db.query('surveys').withIndex('by_municipality_status', (q) => q.eq('municipalityId', municipalityId));
  return limit !== undefined ? await q.order('desc').take(limit) : await q.collect();
}

async function querySurveysByMunicipalityQcStatus(
  ctx: QueryCtx,
  municipalityId: Id<'municipalities'>,
  qcStatus: Doc<'surveys'>['qcStatus'],
  limit?: number,
  ...
  const q = ctx.db
    .query('surveys')
    .withIndex('by_municipality_qcStatus', (q) => q.eq('municipalityId', municipalityId).eq('qcStatus', qcStatus));
  return limit !== undefined ? await q.order('desc').take(limit) : await q.collect();
}
...
async function querySurveysByDistrict(
  ...
  const q = status
    ...
    : ctx.db.query('surveys').withIndex('by_district_status', (q) => q.eq('districtId', districtId));
  return limit !== undefined ? await q.order('desc').take(limit) : await q.collect();
}
  ...
  if (args.qcStatus) {
    ...
    const batches = await Promise.all(
      scopedMunis.map((municipalityId) =>
        querySurveysByMunicipalityQcStatus(ctx, municipalityId, args.qcStatus!, LIST_PAGINATED_SCOPE_LIMIT),
      ),
    );
    ...
    rows = await querySurveysByMunicipality(ctx, args.municipalityId, args.status, LIST_PAGINATED_SCOPE_LIMIT);
  } else if (args.districtId) {
    rows = await querySurveysByDistrict(ctx, args.districtId, args.status, LIST_PAGINATED_SCOPE_LIMIT);
    ...
    if (scopedMunis.length > 1) {
      ...
      rows = await querySurveysByMunicipality(ctx, scopedMunis[0]!, args.status, LIST_PAGINATED_SCOPE_LIMIT);
...
export const listPaginated = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (access === 'own' && noExtraFilters) {
      ...
      const codes = await loadMunicipalityCodes(
        ctx,
        page.page.map((r) => r.municipalityId),
      );
      const enriched = enrichSurveyPropertyIds(page.page, codes);
      const withNames = await enrichSurveyorNames(ctx, enriched);
      return {
        page: withNames,
    ...
    const municipalityPaginateReady =
      args.municipalityId &&
      !args.searchTerm?.trim() &&
      !args.fromMs &&
      !args.toMs &&
    ...
    if (municipalityPaginateReady) {
      const base = args.qcStatus
        ? ctx.db
            .query('surveys')
            .withIndex('by_municipality_qcStatus', (q) =>
...
export const remove = mutation({
  ...
  handler: async (ctx, args) => {
    const [me, survey] = await Promise.all([requireUser(ctx), ctx.db.get(args.id)]);
    if (!survey) return;
    const isOwner = survey.surveyorId === me._id;
    const canDelete = isOwner || (await hasCapability(ctx, me, 'surveys.delete'));
    if (!canDelete) {
      clientError('FORBIDDEN', 'Not your survey');
    }
    await assertMunicipalityInScope(ctx, me, survey.municipalityId);
