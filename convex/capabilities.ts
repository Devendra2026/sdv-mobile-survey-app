...
export async function createCapabilityChecker(ctx: Ctx, user: Doc<'users'>) {
  let cached: Set<string> | undefined;
  const perms = async () => {
    if (!cached) cached = await permissionsForRole(ctx, user.role);
    return cached;
  };
  return {
    has: async (capability: string) => (await perms()).has(capability),
    require: async (capability: string) => {
      if (!(await perms()).has(capability)) {
